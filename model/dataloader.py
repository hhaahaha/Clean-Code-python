...
def get_data(data_path):
    ...
    # 리뷰 문장 추출
    sentences = _data['document']
    # 라벨 추출
    labels = _data['label'].values
    # BERT의 입력 형식에 맞게 변환
...
class makeTrainDataset(makeDataset):
    def split_training_data(input_ids, attention_masks, labels, seed_val=1234):
        ...
        _input = train_inputs, validation_inputs
        _label = train_labels, validation_labels
        _masks = train_masks,validation_masks

        return _input, _label, _masks


class makeTetDataset(makeDataset):
...
class makeTrainDataloader(makeDataloader):
...
class makeTestDataloader(makeDataloader):
